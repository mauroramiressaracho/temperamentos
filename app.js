...
const q = (dimension, text, reverse = false) => ({ dimension, text, reverse });
...
$('profile-form').addEventListener('submit', e => {
  ...
  localStorage.setItem('temperamentos-profile', JSON.stringify(profile));
...
function renderQuestion(){
  ...
  $('question-dimension').textContent = 'Autopercepção';
  ...
  document.querySelectorAll('.answer-option').forEach(btn => btn.onclick = () => {
    ...
    localStorage.setItem('temperamentos-answers', JSON.stringify(answers));
...
function reset(){ localStorage.removeItem('temperamentos-answers'); localStorage.removeItem('temperamentos-profile'); answers=Array(QUESTIONS.length).fill(null); current=0; profile={name:'',age:''}; $('profile-form').reset(); showScreen('intro'); }
...
function dimensionScore(dim){
  ...
  const total = indices.reduce((sum,i)=>{
    const raw=answers[i]; const value=QUESTIONS[i].reverse ? 6-raw : raw; return sum+value;
  },0);
  return Math.round(((total-indices.length)/(indices.length*4))*100);
}

function band(score){ if(score<35)return 'baixo'; if(score<45)return 'moderadamente baixo'; if(score<=55)return 'equilibrado'; if(score<=65)return 'moderadamente alto'; return 'alto'; }
function escapeHtml(text){ return String(text).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c])); }

function calculate(){
  const E=dimensionScore('E'), N=dimensionScore('N'), R=dimensionScore('R'), A=dimensionScore('A'), S=dimensionScore('S'), C=dimensionScore('C'), D=dimensionScore('D');
  const temperaments = {
    Sanguíneo: Math.round((E + (100-N))/2),
    Colérico: Math.round((E + N)/2),
    Melancólico: Math.round(((100-E) + N)/2),
    Fleumático: Math.round(((100-E) + (100-N))/2)
  };
  const ranking = Object.entries(temperaments).sort((a,b)=>b[1]-a[1]);
  return {E,N,R,A,S,C,D,temperaments,primary:ranking[0][0],secondary:ranking[1][0],ranking};
}

function paragraphFor(result){
  const e = result.E < 45 ? 'mais seletiva e reservada' : result.E > 55 ? 'mais expansiva e orientada ao contato' : 'equilibrada entre contato e recolhimento';
  const n = result.N > 60 ? 'com respostas emocionais intensas diante de pressão e incerteza' : result.N < 40 ? 'com boa estabilidade emocional diante de mudanças' : 'com reatividade emocional em faixa intermediária';
  const a = result.A > 60 ? 'Você tende a assumir direção e se posicionar quando percebe necessidade de decisão.' : result.A < 40 ? 'Sua assertividade tende a aparecer de forma mais cuidadosa e seletiva.' : 'Sua assertividade varia conforme o contexto e a importância do problema.';
...
function renderReport(r){
  ...
  const temperCards=r.ranking.map(([name,value],i)=>`<div class="temper-card"><small>${i===0?'Afinidade principal':i===1?'Influência complementar':'Afinidade adicional'}</small><strong>${name}</strong><span>${value}/100</span></div>`).join('');
  ...
  $('report').innerHTML = `
    <section class="report-cover">
      ...
      <div class="cover-meta"><span>${name}</span><span>${age} anos</span><span>68 respostas</span><span>${new Date().toLocaleDateString('pt-BR')}</span></div>
    </section>
    <div class="report-grid">
      ...
      <div class="report-card full"><h2>Leitura geral</h2><p>${paragraphFor(r)}</p><p>O resultado não coloca você em uma caixa rígida. As quatro afinidades existem simultaneamente; o que muda é a força relativa com que certos padrões aparecem nas suas respostas.</p></div>
    </div>
    <div class="report-grid pdf-page-break">
      ...
      ${insight('Intensidade emocional',r.N,'Pressão, críticas e incertezas podem mobilizar rapidamente sua atenção. Isso pode ampliar percepção e envolvimento, mas pede estratégias claras de recuperação.','Seu padrão sugere maior estabilidade diante de contratempos, com tendência a recuperar o equilíbrio com relativa rapidez.','Sua reatividade aparece em faixa intermediária: algumas situações mobilizam bastante, outras são absorvidas sem grande impacto prolongado.')}
      ${insight('Ruminação',r.R,'Sua mente tende a continuar trabalhando depois que um evento termina. Essa profundidade pode favorecer aprendizagem e antecipação, desde que exista um ponto de encerramento para a reflexão.','Você tende a encerrar assuntos mentalmente com maior facilidade e a não revisitar excessivamente eventos passados.','Você alterna entre análise profunda e capacidade de soltar assuntos; vale observar em quais contextos a reflexão deixa de acrescentar algo novo.')}
      ${insight('Assertividade',r.A,'Você tende a se posicionar, decidir e assumir direção quando percebe ausência de clareza. O cuidado é diferenciar firmeza de urgência acumulada.','Você costuma ponderar antes de ocupar espaço ou confrontar problemas. Pode ser útil comunicar incômodos antes que se acumulem.','Sua iniciativa parece situacional: você não precisa liderar o tempo todo, mas pode assumir a frente quando considera necessário.')}
    </div>
    <div class="report-grid pdf-page-break">
      ${insight('Autorregulação',r.S,'Compromissos, deveres e consequências concretas parecem sustentar sua disciplina. Isso favorece confiabilidade, especialmente em situações difíceis.','Sua consistência pode depender mais de motivação e contexto. Estruturas simples, prazos visíveis e redução de atrito podem ajudar.','Você demonstra recursos razoáveis para sustentar compromissos, com oscilações naturais quando propósito ou energia diminuem.')}
      ${insight('Cooperação',r.C,'Você tende a considerar o impacto das decisões e buscar reciprocidade. O desafio é colaborar sem assumir responsabilidades que pertencem aos outros.','Você tende a priorizar autonomia e objetividade. Vale observar se, sob pressão, isso reduz espaço para escuta e negociação.','Você equilibra consideração pelo outro com preservação dos próprios limites, variando conforme a situação.')}
      ${insight('Adaptabilidade',r.D,'Você tende a ajustar rota, estratégia e expectativa quando os fatos mudam. Essa flexibilidade pode ser uma força em ambientes dinâmicos.','Mudanças abruptas podem exigir mais tempo de processamento. Antecipação, planos alternativos e comunicação prévia tendem a reduzir desgaste.','Você consegue se adaptar, embora provavelmente funcione melhor quando mudanças importantes chegam com algum contexto e preparação.')}
      <div class="report-card"><h3>Sob pressão</h3><p>${r.N>55?'A carga interna pode crescer antes de aparecer externamente.': 'Você tende a preservar relativa estabilidade interna.'} ${r.A>55?'Sua resposta provável é transformar parte dessa tensão em decisão e ação.':'Você pode precisar de algum tempo antes de se posicionar.'} ${r.R>60?'Depois, o assunto pode continuar sendo processado mentalmente mesmo quando já terminou por fora.':''}</p></div>
    </div>
    <div class="report-grid pdf-page-break">
      ...
      <div class="report-card"><h2>Pontos para explorar</h2><ul>${r.R>60?'<li>Criar um ponto de encerramento para pensamentos repetitivos.</li>':''}${r.N>60?'<li>Separar competência externa de necessidade interna de recuperação.</li>':''}${r.A>60?'<li>Comunicar limites antes que a urgência se transforme em irritação.</li>':''}${r.D<45?'<li>Antecipar mudanças e trabalhar com planos alternativos.</li>':''}${r.E<45?'<li>Proteger períodos de recarga depois de alta demanda social.</li>':''}</ul></div>
      <div class="report-card full"><h2>Trabalho e liderança</h2><p>Seu melhor ambiente tende a ser aquele em que suas características predominantes encontram propósito e critérios claros. ${r.A>55?'Você pode funcionar bem em situações que exigem decisão, priorização e resolução de problemas.':''} ${r.R>55?'Atividades de análise, planejamento e revisão podem aproveitar sua profundidade, desde que exista limite para o excesso de processamento.':''} ${r.S>55?'A autorregulação favorece funções com responsabilidade por acompanhamento e entrega.':''}</p></div>
      <div class="report-card full"><h2>Relacionamentos e comunicação</h2><p>${r.C>55?'Você tende a considerar reciprocidade e o impacto das próprias decisões.':'Sua comunicação tende a preservar autonomia e objetividade.'} ${r.R>60?'Como você pode processar conversas por mais tempo, é útil diferenciar uma pausa consciente de um afastamento indefinido.':''} ${r.A>55?'Quando algo é importante, sua firmeza pode aumentar; explicitar intenção e limites reduz a chance de a objetividade ser percebida como dureza.':''}</p></div>
    </div>
    <div class="report-grid pdf-page-break">
      <div class="report-card full"><h2>Próximos passos</h2><ol><li>Observe por duas semanas quais situações mais consomem ou restauram sua energia.</li><li>Após conversas difíceis, registre: o que aconteceu, o que está sob seu controle, qual ação ainda existe e o que pode ser encerrado.</li><li>Peça feedback a duas pessoas confiáveis sobre sua forma de decidir, ouvir e se posicionar.</li><li>Compare seu comportamento em contextos diferentes; temperamento descreve tendência, não destino.</li></ol></div>
      <div class="report-card full"><h2>Como ler este resultado</h2><p>Este relatório foi calculado diretamente a partir das 68 respostas. Os escores mostram afinidades dentro deste questionário e não percentis da população. O instrumento usa os eixos Extroversão e Intensidade Emocional para construir os quatro quadrantes temperamentais e acrescenta facetas próprias de ruminação, assertividade, autorregulação, cooperação e adaptabilidade.</p><p class="quote">Este material é destinado a autoconhecimento e entretenimento. Não é teste psicológico validado, não substitui avaliação profissional e não deve ser usado para diagnóstico, seleção clínica ou decisões de saúde.</p></div>
    </div>`;
}

function finishQuiz(){
  if(answers.some(v=>v===null)) return;
  const result=calculate(); renderReport(result); showScreen('result');
}

$('download-pdf').onclick = async () => {
  const btn=$('download-pdf'); const old=btn.textContent; btn.textContent='Gerando PDF...'; btn.disabled=true;
  try{
    const report=$('report');
    const canvas=await html2canvas(report,{scale:1.35,useCORS:true,backgroundColor:'#f4f1ea',windowWidth:1200});
    const {jsPDF}=window.jspdf; const pdf=new jsPDF('p','mm','a4');
    const pageW=210, pageH=297, imgW=pageW, imgH=canvas.height*imgW/canvas.width;
    const img=canvas.toDataURL('image/jpeg',0.92);
    let y=0, remaining=imgH;
    pdf.addImage(img,'JPEG',0,y,imgW,imgH,'','FAST'); remaining-=pageH;
    while(remaining>0){ y-=pageH; pdf.addPage(); pdf.addImage(img,'JPEG',0,y,imgW,imgH,'','FAST'); remaining-=pageH; }
    const safe=(profile.name||'relatorio').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9]+/g,'-').replace(/^-|-$/g,'').toLowerCase();
    pdf.save(`temperamento-${safe}.pdf`);
  }catch(err){ console.error(err); alert('Não foi possível gerar o PDF. Tente novamente ou use a opção de impressão do navegador.'); }
  finally{ btn.textContent=old; btn.disabled=false; }
};

(function restore(){
  try{
    const p=JSON.parse(localStorage.getItem('temperamentos-profile')||'null');
    const a=JSON.parse(localStorage.getItem('temperamentos-answers')||'null');
    if(p?.name){ profile=p; $('full-name').value=p.name; $('age').value=p.age||''; }
    if(Array.isArray(a)&&a.length===QUESTIONS.length){ answers=a; const first=answers.findIndex(v=>v===null); if(first===-1&&p?.name){ renderReport(calculate()); showScreen('result'); } else if(p?.name&&answers.some(v=>v!==null)){ current=Math.max(0,first); showScreen('quiz'); renderQuestion(); } }
